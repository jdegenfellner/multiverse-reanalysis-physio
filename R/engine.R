...
harmonise <- function(d, reg) {
  ...
  # Langformat: Baseline aus der Zeile zum Baseline-Zeitpunkt holen, ueber die ID
  base_long <- NULL
  if (!is.na(reg$time_var) && reg$time_var != "") {
    bt <- if ("baseline_time" %in% names(reg)) reg$baseline_time else NA
    if (!is.na(bt) && bt != "" && (is.na(reg$baseline_var) || reg$baseline_var == "") && !is.na(reg$id_var) && reg$id_var != "") {
      dB <- d[as.character(d[[reg$time_var]]) == as.character(bt), , drop = FALSE]
      base_long <- data.frame(id = dB[[reg$id_var]], b = suppressWarnings(as.numeric(dB[[reg$outcome_var]])))
    }
  ...
  if (!is.null(base_long)) out$.base <- base_long$b[match(out$.id, base_long$id)]
...
fit_mi <- function(d, f, spec, reg, arm_term, seed, cache = NULL) {
  ...
  # Ein Imputationssatz je Studie und Datenzustand (Transformation), gemeinsam fuer alle
  # Spezifikationen; sonst zaehlt Monte-Carlo-Rauschen als Spezifikationsvariation.
  key <- paste0("t=", if (is.null(spec$transform)) "none" else spec$transform)
  if (!is.null(cache) && !is.null(cache[[key]])) imp <- cache[[key]] else {
    imp <- mice::mice(di, m = MI_M, method = "pmm", seed = seed, printFlag = FALSE)
    if (!is.null(cache)) cache[[key]] <- imp
  }
...
run_spec <- function(spec, reg, d0, seed = 1, mi_cache = NULL) {
  d <- d0
  ...
  res <- try(
    if (use_mi) fit_mi(d, f, spec, reg, arm_term, seed, mi_cache) else fit_one(d, f, spec, arm_term),
    silent = TRUE)
...
run_study <- function(reg, which = "defensible", seed = 20260828) {
  ...
  out <- vector("list", nrow(g)); mi_cache <- new.env()
  for (i in seq_len(nrow(g))) {
    r <- run_spec(as.list(g[i, , drop = FALSE]), reg, d0, seed = seed, mi_cache = mi_cache)
  ...
  tq <- ifelse(is.na(res$df) | res$df <= 0, 1.96, stats::qt(0.975, pmax(res$df, 1)))
  res$ci_lo     <- res$est - tq * res$se
  res$ci_hi     <- res$est + tq * res$se
  res$ci_width_d <- 2 * tq * res$se / denom                 # Intervallbreite, standardisiert (t-Quantil)
