...
add("mulligan_PMC13092435", "Group x Time F, p (repeated-measures ANOVA, 3 arms, n = 43)", "F = 0.983, p = 0.383", sprintf("F(%d,%d) = %.3f, p = %.3f", w$Df[i], w$Df[nrow(w)], w$`F value`[i], w$`Pr(>F)`[i]), "SPSS repeated-measures ANOVA; with two occasions the univariate and multivariate solutions coincide")
b <- a[["Error: ID"]][[1]]; g <- grep("Group", rownames(b))
add("mulligan_PMC13092435", "Group main effect F, p", "not reported as a number", sprintf("F(%d,%d) = %.3f, p = %.3f", b$Df[g], b$Df[nrow(b)], b$`F value`[g], b$`Pr(>F)`[g]), "")
...
add("facial_PMC11164989", "Mann-Whitney U p (SFGS composite change, day 20 - baseline)", "p = 0.002", sprintf("p = %.4f (W = %.0f)", wt$p.value, wt$statistic), "exact test, as SPSS reports for small samples; the asymptotic test with continuity correction gives p = 0.0033")
...
add("clusterset_PMC13140343", "Group x Time interaction p (LMM, Type III, Satterthwaite)", "p = 0.352", sprintf("F(%.0f,%.1f) = %.3f, p = %.3f", an["Group:Time","NumDF"], an["Group:Time","DenDF"], an["Group:Time","F value"], an["Group:Time","Pr(>F)"]), "as described in the paper, Methods 2.4.2")
...
add("phosphatidic_PMC4891923", "Group x Time interaction F, p (2x2 mixed ANOVA, n = 18)", "F(1,16) = 33.30, p = 0.041 (Results); 'main effect F(1,16) = 33.30, p < 0.001' (Abstract)", sprintf("F(%d,%d) = %.2f, p = %.4f", w2$Df[i2], w2$Df[nrow(w2)], w2$`F value`[i2], w2$`Pr(>F)`[i2]), "internally inconsistent in the paper: F = 33.30 with df (1,16) implies p < 0.001, whereas p = 0.041 implies F = 4.9")
