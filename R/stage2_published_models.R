...
res <- tryCatch({
  ...
  # Paper: Dreigruppen-LMM (eTRE, dTRE, Control), dummy-kodiert mit Control als Referenz.
  dat$Group <- factor(dat$Group, levels = c("Control", "eTRE", setdiff(unique(dat$Group), c("Control", "eTRE"))))
  ...
  pc <- as.data.frame(pairs(emm, reverse = TRUE))
  pc_post <- pc[pc$Time == "post" & grepl("eTRE", pc$contrast) & grepl("Control", pc$contrast), ][1, ]
...
res <- tryCatch({
  ...
  # Das Paper berichtet fuer den Woche-3-Kontrast MD -1.20 (95% CI -2.18 bis -0.22, p = 0.018); die
  # Bonferroni-Familie ist im Paper nicht definiert, daher wird der unadjustierte Kontrast-p berichtet
  # und der Interaktions-F in deviations dokumentiert.
  p_bonf <- t_res$p.value
  f_int <- tryCatch({ sm <- summary(aov_fit); sm[["Error: id:Time"]][[1]] }, error = function(e) NULL)
...
res <- tryCatch({
  ...
  # SPSS-GLM-Repeated-Measures-ANCOVA (listenweiser Ausschluss): der Zwischen-Subjekt-Effekt
  # der Gruppe entspricht der ANCOVA auf dem Mittel der beiden Messwiederholungen.
  cc <- dat %>% filter(!is.na(SPADItot4wks) & !is.na(SPADI8wks) & !is.na(SPADItotPre)) %>%
    mutate(SPADI_mean48 = (SPADItot4wks + SPADI8wks) / 2)
  m1 <- lm(SPADI_mean48 ~ Group + SPADItotPre, data = cc)
  print(summary(m1))
  cf <- summary(m1)$coefficients
  ci_m <- confint(m1)["GroupIS", ]
  long <- cc

  add_result(
    "spadi_PMC4880881",
    "Repeated-measures ANCOVA als GLM-Aequivalent: ANCOVA auf dem Mittel der Wochen 4 und 8 ~ Group + SPADI-Baseline, listenweiser Ausschluss; entspricht dem Zwischen-Subjekt-Effekt der SPSS-GLM-RM-ANCOVA. eline + (1|IDnr); Kontrast IS (Group=1) vs TAU (Group=3)",
...
res <- tryCatch({
  ...
  m1 <- lmer(sixmwd ~ Time + grp_post + grp_fu + center + (1 | num_ID), data = long, REML = FALSE)  # Stata mixed: ML-Voreinstellung
  print(summary(m1))
  cf <- summary(m1)$coefficients
...
res <- tryCatch({
  ...
  dat$weekF <- factor(dat$week)   # Paper: "intervention effects at specific follow-up time points" = Zeit als Faktor

  fit_full <- function(random_part) {
    frm <- as.formula(paste0(
      "rmdq_tot ~ rmdq_tot_base + weekF * Tment + age_w0 + gender_w0 + NZDep_Decile + ",
  ...
  emm <- emmeans(m1, ~ Tment | weekF)
  pc <- as.data.frame(pairs(emm, reverse = TRUE)); pc <- pc[pc$weekF == "26", ]  # FREE - Control bei Woche 26
