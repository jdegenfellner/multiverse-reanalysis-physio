...
short <- function(x) sub("_PMC[0-9]+$", "", x)
md <- function(df, digits = 3) {
  if ("study_id" %in% names(df)) df$study_id <- short(df$study_id)
...
ex <- read.csv("config/exclusions.csv", stringsAsFactors = FALSE); ex$study_id <- sub("_PMC.*$", "", ex$study_id); if ("reason_en" %in% names(ex)) ex$reason <- ex$reason_en; ex$reason <- substr(ex$reason, 1, 160)
...
st2 <- read.csv("output/stage2_published_models.csv", stringsAsFactors = FALSE); if ("deviations_en" %in% names(st2)) { st2$deviations <- st2$deviations_en; st2$model_as_published <- st2$model_as_published_en }; st2$deviations <- substr(gsub("\n", " ", st2$deviations), 1, 220); st2$model_as_published <- substr(st2$model_as_published, 1, 160)
...
rg <- read.csv("config/registrations.csv", stringsAsFactors = FALSE); if ("registered_primary_outcome_en" %in% names(rg)) rg$registered_primary_outcome <- rg$registered_primary_outcome_en; rg$registered_primary_outcome <- substr(rg$registered_primary_outcome, 1, 140)
...
nov <- readLines("literature/novelty_search_2026-09-15_en.md"); nov <- sub("^# ", "### ", nov); nov <- sub("^## ", "### ", nov)
