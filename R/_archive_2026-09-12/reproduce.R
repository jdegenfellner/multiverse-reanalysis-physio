...
for (i in seq_len(nrow(pub))) {
  ...
  for (nd in names(target)) {
    if (is.na(target[[nd]]) || !(nd %in% names(x))) next
    k <- k + 1
  ...
  best <- x[sc == max(sc), ]
  if (nrow(best) > 1) best <- best[order(abs(best$est - p$published_est)), ][1, ]  # bei Gleichstand naechster Wert
  matched_all <- max(sc) == k
  rel <- abs(best$est - p$published_est) / abs(p$published_est)
  abs_d <- abs(best$est - p$published_est) / x$sd_denom[1]           # absolute Differenz in SD-Einheiten
  same_sign <- sign(best$est) == sign(p$published_est)
  # Klassifikation: relative Differenz (Hardwicke 2021: minor < 10 %, major >= 10 %) ODER
  # absolute Differenz in SD-Einheiten, weil die relative Regel bei Schaetzern nahe 0
  # instabil ist (Statistikgutachten 2026-09-12, Befund 3). Volle Reproduktion 2 % oder 0.02 SD.
  cls <- if (same_sign && (rel <= 0.02 || abs_d <= 0.02)) "full" else
         if (same_sign && (rel <= 0.10 || abs_d <= 0.05)) "minor_discrepancy" else "major_discrepancy"
  pd <- p$published_est / x$sd_denom[1]
  ...
  out[[i]] <- data.frame(study_id = sid, class = cls, spec_fully_matched = matched_all,
    ...
    published_est = p$published_est, our_est = best$est, rel_diff = rel, abs_diff_sd = abs_d,
    ...
    published_d = pd,
    pct_principled = mean(xp$d <= pd), pct_defensible = mean(x$d <= pd),
    inside_principled = pd >= min(xp$d) & pd <= max(xp$d),
    inside_defensible = pd >= min(x$d) & pd <= max(x$d))
}
res <- bind_rows(out)
write.csv(res, "output/reproduction.csv", row.names = FALSE)
cat("\n=== Stufe 2: Reproduktion ===\n")
print(table(res$class))
cat("publizierte Spezifikation vollstaendig im Gitter:", sum(res$spec_fully_matched, na.rm = TRUE), "von", sum(!is.na(res$spec_fully_matched)), "\n")
