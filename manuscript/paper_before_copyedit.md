# Reproducibility without a mandate: computational reproduction and multiverse analysis of exercise, rehabilitation and physiotherapy trials with publicly deposited individual participant data

*Draft of 2026-09-13, revised after the adversarial review (`manuscript/review_adversarial.md`). Verification of this version pending. Citation keys refer to `literature/references.bib`. Every number is taken from
...
**Results.** Sixteen trials were included, 11% of those with a link; 104 of 136 losses
occurred before the data were opened, and of the 32 after, 11 were deposits that could
...
standard deviations in the median trial and by 0.41 at most, and no specification
reversed its sign; in 11 of 16 trials only baseline handling could vary, and the
...
logarithmic transformation, and reversed the published sign in single specifications of
...
estimates the original authors called "statistically significant" remained
"significant" with the same sign under robustness checks chosen by the reproducers [Brodeur_2026]. A policy that is not verified
...
carried one, and the figure depends on journal policy, because one journal that requires
...
[Elghzali_2025]. In the journals of four national physiotherapy
associations, 12% of 465 articles made data directly available, and the authors recommend
a default open data policy that the field does not yet have [Jabouille_2025]. When authors of rehabilitation studies whose availability statements
...
[Dhanani_2022]. I am not aware of a study in physiotherapy, rehabilitation or exercise
...
non-equivalent choices add, and through which choices? I also record which reporting
features of the papers obstructed reproduction, and I report how often my own first
...
1.08 standard deviations in one trial; and the core grid was renamed from principled to
estimand-preserving once the precision criterion of Del Giudice and Gangestad had been
applied to baseline handling. All three are stated where they matter, and all results
...
I searched Europe PMC on [date, check README] for open-access articles with
...
the one the paper calls primary; failing that, the outcome of the sample-size
calculation; failing that, the first outcome reported in the abstract. The source is
recorded for each trial, and a paper whose headline outcome is not the registered primary
...
phosphatidic acid trial the value first extracted was an F statistic, and the trial was
...
the paper describes it, in a separate script outside the multiverse grid: analysis of
covariance where the paper reports one; a linear mixed model with random intercept and
slope for practitioner and the prespecified covariates present in the deposit for the
cluster-randomised FREE trial; a mixed model over all four measurement occasions on the
logarithmic scale, with the authors' deposited code, for the balance feedback trial; a
constrained longitudinal analysis for TERECO; a Bayesian multilevel model with default
priors, because the paper states none, for the deload trial; multiple imputation with 20
imputations, because the paper does not state the number, for the water exercise trial;
and a repeated-measures analysis of covariance over the two short-term follow-ups,
approximated as a mixed model, for the frozen shoulder trial. Where a detail was not
...
digit, a minor discrepancy at most 10%, a major discrepancy otherwise [Hardwicke_2021;
Naudet_2018]. The absolute difference in pooled standard
...
reported, and the percentile of the published estimate in the distribution of the
...
baseline variables are rounded to varying degrees, which I did not correct
...
### 2.6 Stage 3: the grids

The multiverse varies six decision nodes (Table 1). Following Del Giudice and Gangestad,
each node was classified before computation as Type E, where the options have comparable
validity, examine the same effect and estimate it with comparable precision; Type N,
where one option has justified precedence or the options examine different effects; or
Type U, where equivalence is uncertain [Del_Giudice_2021]. I depart from this scheme in
...
outcome are correlated [Vickers_2001; Senn_2006; Van_Breukelen_2006]. Under a strict
...
the criterion the grid is named for, and I call that grid estimand-preserving; the
...
covariate set (none, the stratification factors, or the published set) [Kahan_2014;
Kahan_2012], the model family (linear model, or a random intercept for a cluster variable
...
of Short et al. [Short_2026], adds two Type N nodes: the missing-data strategy (complete
cases; multiple imputation by predictive mean matching with 20 imputations, Rubin's rules
and Barnard and Rubin degrees of freedom; or last observation carried forward where a
baseline exists), which is Type N on both criteria, because complete cases and imputed data
target different populations once dropout is informative and because imputation has
justified precedence under a missing-at-random assumption [White_2011]; and logarithmic
transformation of outcome and baseline, which estimates a ratio of geometric means. The difference between the
...
literature, and winsorising changes the quantity estimated. The node was reclassified as
...
collapsed to one, and the share of collapsed duplicates is reported. Non-applicable nodes
...
grid I report the median, interquartile range and range of d over unique specifications; the share of specifications whose sign agrees with the median;
...
deposit could not support the published analysis, and in eight of them the data were
...
multiverse ranged from 16 to 276, and the fitted published models used 20 to 373
observations.
...
published estimate, and in the 8 trials that report a p-value and for which a
...
F(1,16) = 33.30, does not: the interaction F is 4.95, and no term in the analysis gives
33.30; the abstract reports the same statistic as a main effect with p < 0.001. The
published p-values therefore reproduce in all four, and one F statistic is misreported.

Counting every study with a repository link as the denominator, as Siebert et al. did for all sampled trials [Siebert_2022], the published primary estimate was recovered in 11 of
152 (7%, Wilson interval 4 to 12%); counting the 48 assessed trials, in 11 of 48 (23%,
13 to 37%).

Located in the estimand-preserving multiverse, the published estimate lies inside the
range in 9 of the 11 trials on a common scale (Wilson interval 52 to 95%), and the
...
estimable; the covariate set varied in 4 trials and the model family in 2, and in one
...
The same picture in terms of the distribution of d: its range per trial has a median of
0.12 standard deviations (bootstrap 95% interval 0.09 to 0.21; interquartile range 0.08
to 0.22) and a maximum of 0.41, and the sign of d is the same in every specification in
...
correlated above 0.99, and in one trial only one pipeline existed; sign stability in
...
standard deviations; 95% and 91% of their specifications keep the published sign. The
...
trial. The sign was stable in 13 of 16 trials (57 to 93%), and the range includes zero in
...
from 1.4 to 0.4. No trial in the corpus applied such a rule, and the arm is reported as
an illustration of what an option outside the literature does to a bounded outcome, not
as fragility.

### 3.5 Reporting, and the effect of my own mapping
...
report tests of baseline balance and their "significance", and in three the result of
that test
...
0.15 standard deviations in the typical trial and by 0.41 at most, and never changed
sign; in 11 of 16 trials the only such choice that could move it was the handling of the
baseline value, and with that choice fixed the range was zero in 12 trials. Adding choices that change
...
and my own first mapping of published analyses onto deposited data reversed the
...
Elghzali_2025]. The second loss is different in kind. In 11 of 48 trials the deposit could not
...
them to the printed decimal, and none differed by more than 0.014 standard deviations;
every recomputed interval contains the published estimate. The interval on the
proportion is wide because the denominator is 12, and the denominator is 12 because only
...
17 reanalyses [Naudet_2018]. The one discrepancy comes from a Bayesian model whose paper
...
### 4.4 Fragility, and what the grid could vary
...
change-score and covariance-adjusted estimates under imbalance predicts [Senn_2006;
Van_Breukelen_2006]. With baseline handling fixed to analysis of covariance, which has
...
estimand-preserving grid by a median factor of 1.35, and the node that most often drove
...
preprocessing [Veltri_2025]; in these trials, once options that change the estimand are
...
showed what a multiverse looks like for one trial [Crenshaw_2026]; applying one classified
...
unchanged, and sign stability in those trials is not evidence of anything. And 196
...
eight; three trials were never registered, and for one registered trial neither the
...
an estimate of the arm difference, and one states what quantity it is estimating. CONSORT
has asked for a defined primary outcome and an effect size with its precision since 2010
[Schulz_2010], and the 2025 revision asks for the metric and the timepoint of every
outcome [Hopewell_2025]; the estimands framework asks for the quantity itself
[ICH_2019_E9R1; Kahan_2024]. Thirteen papers test baseline balance, a practice Senn argued
against in 1994 [Senn_1994], and three let the test decide the adjustment, which makes the
...
trial registered are not the same thing, and that the registration is where the
difference shows.
...
range by much; it changed their sign. These were coding errors, not choices between defensible alternatives, and they belong to
the reproducer, not to the trials. Brodeur and colleagues let reproducers choose their
...
registered as secondary, and four trials name co-primary outcomes of which I analysed
...
inconsistent about which arm is the control, and I followed the design section. The corpus came from one database with one search string, and it required a repository
...
estimand-preserving grid keeps a node whose options differ in precision, and in most
...
for practitioner and practice were singular in every fit, and the linear-model
...
trial. Instrument-specific minimal important differences were not collected, and the 0.2
...
in two steps, and the second, from public data to identifiable analysis, is the one that
...
only such choice was the handling of the baseline value, and without it there was almost
nothing to vary. Choices
that change the estimand widened the range by a third. Three of 12 primary reports do not
identify a primary outcome through registration or paper, and four papers give no
