...
replications, 28% succeeded [Murphy_2025]. Dhanani and colleagues recomputed the statistics of
572 trials from the numbers printed in the articles, without individual data
[Dhanani_2022]. To the best of my knowledge, no published or registered study in physiotherapy,
...
by the authors [Nolte_2026_reg], alongside a preprint by the same group on data and code
availability in sports science [Nolte_2026_preprint]. Reproducibility in this literature usually
...
one where equivalence is uncertain [Del_Giudice_2021]. Short and colleagues separate the
...
[Patel_2015; Klau_2020]. Studies in which many analysts receive the same data have
...
[Brodeur_2026]. The same applies here. Section 3.5 reports what the
...
the paper describes it, independently of the multiverse grid. Six trials required more than a
...
showed that rounding distorts the distribution of these p-values [Bolland_2019]. The
...
[Del_Giudice_2021]. I depart from this scheme in
...
of Short et al. [Short_2026], adds two Type N nodes. The first is the missing-data
...
missing-at-random assumption [White_2011]. The second is logarithmic transformation of
...
continuous measure of evidence against the test hypothesis [Rafi_2020], as is the
...
is then not evidence of robustness [Short_2026]. Descriptive correlates of the principled
...
[Schulz_2010]. The 2025 revision asks for the metric and the timepoint of every
...
against in 1994 [Senn_1994]. Three let the test decide the adjustment, which makes the
...
freedom does to conclusions [Silberzahn_2018; Breznau_2022]. Fixed rules, written before the mapping and applied to every trial, are the remedy that was applied. The error rate of the first pass belongs in the report.
